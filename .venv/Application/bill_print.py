...
def format_bill(job_id, Shop_Name, recieved_date, shop_address,phone_number,product_name,solution):
            bill = f"""
    ************************************
            REPLACEMENT INVOICE
    ************************************
...
def generate_pdf(bill_content):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Courier", size=12)  # Using Courier for better alignment of text
    pdf.multi_cell(0, 10, bill_content)  # Adds the formatted bill content to the PDF
...
action = st.radio("Select Action:", ("Add New Record", "Edit Existing Record", "Search Records", "Print Job"), horizontal=True)
        ...
        if records:
            ...
            for record in records:
                if record['job_id'] == selected_retailer_id:
                    ...
                    break  
            if selected_record:
                # Extract values for the bill
                job_id = selected_record['job_id']
                shop_name = selected_record['shop_name']
                recieved_date = selected_record['recieved_date']
                shop_address = selected_record['shop_address']
                phone_number = selected_record['phone_number']
                product_name = selected_record['product_name']
                solution = selected_record['solution']
                
                # Format the bill
                bill_content = format_bill(job_id, shop_name, recieved_date, shop_address, phone_number, product_name, solution)
                
                # Generate and save PDF
                pdf_path = generate_pdf(bill_content)
                
                # Display the bill in the Streamlit app
                st.text_area("Invoice Preview", value=bill_content, height=300)
                ...
                with open(pdf_path, "rb") as f:
                    st.download_button(
                        ...
                        data=f,
                        file_name=f"{job_id}_invoice.pdf",
                        mime="application/pdf",
                    )
                
                # Print the invoice
                if st.button("Print Invoice"):
                    print_to_printer(pdf_path)
        else:
            st.warning("No records found for the selected date.")
    else:
